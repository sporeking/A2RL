...
class BaseAlgo(ABC):
    ...
    def collect_experiences_mutation(self, 
                                     G, 
                                     get_mutation_score, 
                                     mutation_buffer, 
                                     mutation_value, 
        ...
        for i in range(self.num_frames_per_proc):
            ...
            mutation = obs.image - self.obss[i-1].image
            if get_mutation_score(mutation) > mutation_value:
                ...
                for idx, (score_, mutation_, times_, env_) in enumerate(mutation_buffer):
                    if contrast(mutation, mutation_) > 0.99:
                        mutation_buffer[idx][2] += 1
                        is_in_buffer = True
                        break
                if not is_in_buffer:
                    heapq.heappush(mutation_buffer, (get_mutation_score(mutation), mutation, 1, copy.deepcopy(self.env)))
