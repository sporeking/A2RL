...
BASE_MODEL_NAME="ABL-PPO-taxi-test" # Your model name. 
ENV="Taxi-v0" 
...
for i in $(seq 1 $NUMS); do
  ...
  python discover.py --task-config task2 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $TOTAL_STEPS --seed $i --curriculum 1 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC
done
