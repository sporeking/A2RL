...
anomalyNN = torch.load("StateNN/test_8.pth")
...
class ManualControl:
    ...
    def step(self, action: Actions):
        ...
        if self.current_state != get_state(self.env):
            ...
            self.current_state_img = obs
            self.current_state = get_state(self.env)
            mutation = obs_To_mutation(self.last_state_img, self.current_state_img, preprocess_obss)
            anomaly_mutation = transforms.ToTensor()(mutation.cpu().numpy().astype(numpy.uint8)).cuda().unsqueeze(0)
            print(anomalyNN(anomaly_mutation))
            # mutation = mutation.cpu().numpy().astype(numpy.uint8)
            # timestamp = int(time.time())
            # plt.imsave(f"../test/dataset1/{self.current_state}/image_{timestamp}.bmp", mutation)
            # new_mutation = plt.imread(f"../test/dataset1/{self.current_state}/image_{timestamp}.bmp")
            # print(anomalyNN(transforms.ToTensor()(new_mutation).cuda().unsqueeze(0)))
            # print(numpy.array_equal(mutation, new_mutation))
            # plt.imsave(f"../test/dataset1/{self.current_state}/image_{timestamp}.jpg", mutation)
        else:
            self.last_state_img = self.current_state_img
            self.current_state_img = obs
            self.current_state = get_state(self.env)
            ...
            # mutation = mutation.cpu().numpy().astype(numpy.uint8)
            # timestamp = int(time.time())
            # plt.imsave(f"../test/dataset1/0/image_{timestamp}.bmp", mutation)
            # new_mutation = plt.imread(f"../test/dataset1/0/image_{timestamp}.bmp")
            # print(anomalyNN(transforms.ToTensor()(new_mutation).cuda().unsqueeze(0)))
            # print(numpy.array_equal(mutation, new_mutation))
        # if self.current_state != get_state(self.env):
        #     self.last_state_img = self.current_state_img
